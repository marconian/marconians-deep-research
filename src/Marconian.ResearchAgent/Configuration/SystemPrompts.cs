...
public static class SystemPrompts
{
    ...
    public static class Templates
    {
        public static class Orchestrator
        {
            ...
            public const string OutlineInstruction = "Design a structured outline for the final report. Return JSON with fields: 'title' (string), 'notes' (string), 'sections' (array), and 'layout' (array). Each section entry must include 'sectionId', 'title', 'summary', and 'supportingFindingIds' referencing the provided IDs, plus an optional boolean 'structuralOnly' that defaults to false. When 'structuralOnly' is true the section is purely structural (heading, list, or container) and no narrative paragraphs should be drafted for it. The layout array should describe the Markdown heading hierarchy: every node must include 'nodeId', 'headingType' (h2-h4), 'title', optional 'sectionId' matching a section entry, and 'children' (array). Provide whatever sections are warranted; do not add compulsory segments unless supported by the evidence. Always include a final 'Sources' node (without sectionId) so citations can be appended, and avoid h1 headings because the renderer will inject the report title. No extra commentary.";
